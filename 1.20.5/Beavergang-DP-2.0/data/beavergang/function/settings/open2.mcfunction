...
execute positioned ^ ^-0.2 ^1.4 run summon interaction ~ ~ ~ {height: 1f, width: 0.5f, Tags:[beavergang_settings,newInteraction,get],Passengers:[{id:"minecraft:text_display",text:{"text":"beavergang","italic":"true"},Tags:[beavergang_settings,newInteraction,get, translate2],Passengers:[{id:"minecraft:text_display",text:{"text":"get deck:"},Tags:[beavergang_settings,newInteraction,translate]}]}]}

scoreboard players operation @e[tag=newInteraction] beavergang = @s beavergang

execute as @e[tag=translate,tag=newInteraction] run data modify entity @s transformation.translation[1] set value -0.6f
execute as @e[tag=translate2,tag=newInteraction] run data modify entity @s transformation.translation[1] set value -0.9f
